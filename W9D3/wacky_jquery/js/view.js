...
View.prototype.exercise5 = function () {
  ...
  //your code here!
  $('#easel').on("click", "li.square", function(event) {
    event.preventDefault();
    // event.target seems to return an HTMLElement, so we need to convert it to a jQuery object
    let $square = $(event.target);
    let coords = $square.attr('data-pos');
    alert(coords);
  });
};
...
View.prototype.exercise7 = function(){
  ...
  //your code here!
  $('#easel').on("mouseover", "li.square", function(event) {
    let $square = $(event.target);
